  ...
  async function handleConfirm() {
    try {
      ...
      if (response.data.status === 200) {
        refetch();
        toast.success(response.data.message || vt('Stock type updated successfully'));
        setAlertBox(false);
        setLoading(false);
        onAdd();
      } else {
        throw new Error(response.error.data.message || vt('Failed to update stock type'));
      }
    } catch (error) {
      toast.error(error?.message || error || vt('An error occurred while updating the stock type'));
      setLoading(false);
      setAlertBox(false);
    }
  }
