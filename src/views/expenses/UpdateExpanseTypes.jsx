  ...
  async function handleConfirm() {
    try {
      ...
      if (response.data.status === 200) {
        refetch();
        onAdd();
        toast.success(response.data.message || t('expenseCategoryUpdatedSuccess'));
      } else {
        toast.error(response.data.message || t('failedToUpdateExpenseCategory'));
      }
    } catch (error) {
      toast.error(error?.message || t('failedToUpdateExpenseCategory'));
    } finally {
      setIsLoading(false);
      setLoading(false);
      setAlertBox(false);
