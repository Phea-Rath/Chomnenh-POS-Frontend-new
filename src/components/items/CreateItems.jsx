...
import { useGetAllAttributeQuery } from "@/features/products/attributesSlice";
...
const ItemForm = () => {
  ...
  const {
    ...
    refetch: refetchItem,
  } = useGetItemByIdQuery({ token, id }, { skip: !isEditMode });
  ...
  const saleContext = useGetAllSaleQuery({
    token,
    limit: 10,
    page: 1,
    search: "",
  ...
  const updateColorsInAttributes = (newColors) => {
    const colorAttributeIndex = attributes.findIndex(
      (attr) =>
  ...
  const generateAutoCode = () => {
    ...
    setItem((prev) => ({ ...prev, code: `PRD-${randomNum}` }));
  };
  ...
  const validateForm = () => {
    ...
    if (!item.scale_id || item.scale_id === "") {
      newErrors.scale_id = t("scaleRequired") || "Scale is required";
    }
    ...
    attributes.forEach((attr, index) => {
      ...
      if (
        attr.name !== "colors" &&
        (!attr.value || (Array.isArray(attr.value) && attr.value.length === 0))
      ) {
  ...
  function changeUpload(e) {
    ...
    if (files.length > 0) {
      ...
      if (validFiles.length > 0) {
        const newViewImages = validFiles.map((file) =>
  ...
  return (
    ...
    >
      ...
      <div>
          ...
          <div className="p-4 flex gap-4 items-center justify-between bg-gray-50 dark:bg-gray-600">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                disabled={!definePermission(MENU_ID).is_modify || saving}
        ...
        <form className="max-w-7xl mx-auto p-4 md:p-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            {/* Left Column - Image Upload */}
            <div className="lg:col-span-4 bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-[2px] p-4 shadow-sm h-fit space-y-4">
              <h2 className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 pb-2 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2">
                <LuImage size={14} />
                {t("itemImages") || "Item Images"}
              </h2>
              ...
              <label
                ...
                className={`group relative block cursor-pointer overflow-hidden rounded-[2px] border-2 border-dashed transition-all duration-200 ${
                  errors.images
                    ...
                    : "border-slate-300 dark:border-slate-700 hover:border-[#13b5ea] hover:bg-slate-50 dark:hover:bg-slate-800/50"
                }`}
              >
              ...
              {(existingImages.length > 0 || viewImages.length > 0) && (
                <div className="grid grid-cols-3 gap-2 pt-2">
                  {existingImages.map((image, index) => (
                    <div
                      key={`existing-${index}`}
                      className="group relative aspect-square bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-[2px] overflow-hidden"
                    >
                      <img
                        src={image.image}
                        alt="Existing"
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          removeExistingImage(index, image.image_id)
                        }
                        className="absolute top-1 right-1 h-5 w-5 flex items-center justify-center bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-all text-xs"
                      >
                        <LuTrash2 size={10} />
                      </button>
                    </div>
                  ...
                  {viewImages.map((image, index) => (
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-[#13b5ea] text-white text-[8px] font-bold uppercase text-center py-0.5">
                        {t("new")}
                      </div>
                    </div>
                  ))}
            ...
            <div className="lg:col-span-8 flex flex-col gap-6">
              {/* Basic Information */}
              <section className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-[2px] p-5 shadow-sm space-y-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        ...
                        className="text-[10px] font-bold uppercase text-[#13b5ea] hover:underline flex items-center gap-1"
                      >
                        <LuZap size={12} /> Auto Generate
                      </button>
                    </div>
                    <Input
                ...
                <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                      Pricing & Profit Insights
                    </span>
                    {parseFloat(item.price) > 0 && parseFloat(item.cost) > 0 && (
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-0.5 text-[10px] font-bold rounded-[2px] uppercase ${
                          parseFloat(item.price) - parseFloat(item.cost) >= 0 
                            ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400' 
                            : 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400'
                        }`}>
                          Margin: {(((parseFloat(item.price) - parseFloat(item.cost)) / parseFloat(item.price)) * 100).toFixed(1)}%
                        </span>
                        <span className="px-2 py-0.5 text-[10px] font-bold rounded-[2px] bg-cyan-100 text-cyan-700 dark:bg-cyan-900/40 dark:text-cyan-400 uppercase">
                          Profit: ${(parseFloat(item.price) - parseFloat(item.cost)).toFixed(2)} / unit
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <Input
                        onChange={(value) => setItem({ ...item, cost: value })}
                        value={item.cost}
                        type="number"
                        placeholder="0.00"
                        addonBefore="$"
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <Input
                        onChange={(value) =>
                          setItem({ ...item, wholesale_price: value })
                        }
              ...
              <section className="bg-gray-50 dark:bg-gray-800/50 ">
                ...
                <div className="p-4">
                  {attributes
                    ...
                    .map((attribute, index) => {
                      const actualIndex = attributes.findIndex(
                        (a) => a === attribute,
                      );
                      return (
                        <div
                          key={actualIndex}
                          className="group relative grid gap-4 md:grid-cols-12 items-end bg-slate-50/50 dark:bg-slate-800/30 p-4 rounded-[2px] border border-slate-100 dark:border-slate-800/50"
                        >
                          <div className="md:col-span-5 flex flex-col gap-1.5">
                            ...
                            <Select
                              ...
                              onChange={(value) =>
                                updateAttribute(actualIndex, "name", value)
                              }
                              dropdownRender={(menu) => (
                                <div>
                                  {menu}
                  ...
                  {attributes.filter((attr) => attr.name !== "colors")
                    .length === 0 && (
