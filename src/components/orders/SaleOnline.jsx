          ...
          {orders?.items?.length > 0 && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {/* Customer Phone */}
                <div className={`flex items-center justify-between gap-4 ${orders?.sale_type !== "sale" ? "hidden" : ""}`}>
                  <label className="label text-gray-600">លេខទូរស័ព្ទអ្នកទទួល</label>
                  <input
                    onChange={(e) => {
                      setOrders((prev) => {
                        const results = {
                    ...
                    className={`input bg-transparent text-gray-800 border-gray-400 focus:outline-none rounded transition-colors ${validationErrors.order_tel
                  ...
                  <textarea
                    onChange={(e) => {
                      setOrders((prev) => {
                        ...
                        // console.log(typeof Number(e.target.value.split(",")[0]?.trim()));

                        if (Number(e.target.value.split(",")[0]?.trim())) {
                          setLocation({
                            latitude: e.target.value.split(",")[0]?.trim(),
                            longitude: e.target.value.split(",")[1]?.trim(),
                          });
                        }
                        return results;
                      });
                    ...
                    className={`w-full px-2 py-1 border rounded text-sm transition-colors ${validationErrors.order_address
                      ? "border-red-500 border-2"
                      : "border-gray-300"
                      }`}
                    placeholder="address"
