...
import { useGetProductionReportMutation } from "@/features/dashboard/reportsSlice";
...
const ProductionReport = () => {
    ...
    async function fetchReport(payload = formData) {
        try {
            setLoading(true);
            const res = await getProductionReport({ itemData: payload, token });
            if (res?.data?.status === 200) {
                setReportData(res.data.data || []);
            } else {
                toast.error('Failed to generate production report');
            }
        } catch (error) {
    ...
    const handleExportExcel = () => {
        ...
        const exportData = reportData.flatMap((production) => {
            const details = production.details?.length ? production.details : [null];

            return details.map((detail) => ({
                'Production No': production.production_no,
                'Production Date': production.production_date,
                Barcode: production.barcode,
                Item: production.item_name,
                'Item Code': production.item_code,
                'Production Qty': Number(production.quantity) || 0,
                'Production Cost': Number(production.total_cost) || 0,
                'Created By': production.created_by,
                'Raw Material': detail?.raw_material_name || '-',
                'Raw Code': detail?.raw_material_code || '-',
                'Raw Qty': detail ? Number(detail.quantity) || 0 : 0,
                'Raw Unit Cost': detail ? Number(detail.cost_per_unit) || 0 : 0,
                'Raw Total Cost': detail ? Number(detail.total_cost) || 0 : 0,
            }));
        });
    ...
    return (
        <div className="report-page min-h-screen bg-transparent p-2 md:p-4">
            ...
            <div className="max-w-7xl mx-auto">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{rt("Production Report")}</h1>
                    <p className="text-slate-500 text-sm mt-1 dark:text-slate-400">{rt("Generate and export production reports")}</p>
                </div>
                ...
                <div className="bg-primary rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-5 text-xs mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                        ...
                        <div>
                            ...
                            <RichSearch
                                data={items}
                                keyFields={{ id: 'id', title: 'item_name' }}
                        ...
                        <div className="overflow-x-auto print:overflow-visible print:p-0 print:bg-white print:text-black print:shadow-none print-container" ref={reportRef}>
                            ...
                            <div className="hidden print:flex items-center justify-between mb-8 border-b pb-4 print-header">
                                ...
                                <div className="text-right">
                                    <h1 className="text-xl font-bold text-cyan-600">{rt("Production Report")}</h1>
                            ...
                            <table className="min-w-full border-collapse border border-slate-200 dark:border-slate-700">
                                <thead className="bg-slate-50 dark:bg-slate-800">
                                    <tr>
                                        <th className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{rt("Production No")}</th>
                                        ...
                                        <th className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{rt("Item")}</th>
                                        ...
                                        <th className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-right text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{rt("Qty")}</th>
                                        <th className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-right text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{rt("Total Cost")}</th>
                                        ...
                                        <th className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{rt("Raw Materials")}</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-100 dark:divide-slate-800">
                                    {reportData?.map((item, index) => (
                                        <tr key={index} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors align-top">
                                            <td className="border border-slate-200 dark:border-slate-700 px-4 py-3 font-medium text-slate-900 dark:text-slate-100">{item.production_no}</td>
                                            <td className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-slate-600 dark:text-slate-400">{item.production_date}</td>
                                            <td className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-slate-600 dark:text-slate-400">{item.barcode}</td>
                                            ...
                                            <td className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-right text-slate-600 dark:text-slate-400">{formatNumber(item.quantity)}</td>
                                            <td className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-right font-medium text-emerald-600">{formatCurrency(item.total_cost)}</td>
                                            <td className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-slate-600 dark:text-slate-400">{item.created_by}</td>
                                            <td className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-slate-600 dark:text-slate-400 min-w-[300px]">
                                                {item.details?.length > 0 ? (
                                                    <div className="space-y-2">
                                                        {item.details.map((detail, dIndex) => (
                                                            <div key={`${item.production_no}-${dIndex}`} className="border border-slate-100 dark:border-slate-700 rounded-md p-2 bg-slate-50 dark:bg-slate-800">
                                                                <div className="font-medium text-slate-700 dark:text-slate-300 text-[10px]">
                                                                    {detail.raw_material_name} ({detail.raw_material_code})
                                                                </div>
                                                                <div className="text-slate-500 dark:text-slate-400 text-[9px]">
                                                                    {rt("Qty")}: {formatNumber(detail.quantity)} | {rt("Cost")}: {formatCurrency(detail.cost_per_unit)} | {rt("Total")}: {formatCurrency(detail.total_cost)}
                                                                </div>
                                                            </div>
                                                        ))}
                                                    </div>
                                    ...
                                    {reportData.length > 0 && (
                                        <tr className="bg-slate-50 dark:bg-slate-800 font-semibold">
                                            ...
                                            <td className="border border-slate-200 dark:border-slate-700 px-4 py-3 text-right text-emerald-600">{formatCurrency(totals.total_cost)}</td>
                            ...
                            {reportData?.length > 0 && (
                                <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-md border border-slate-100 dark:border-slate-700">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <span className="font-medium text-slate-600 dark:text-slate-400">{rt("Total Productions: ")}</span>
                                            <span className="text-slate-700 dark:text-slate-200">{reportData.length}</span>
                                        </div>
                                        <div>
                                            <span className="font-medium text-slate-600 dark:text-slate-400">{rt("Total Cost: ")}</span>
                                            <span className="text-emerald-600 font-medium">{formatCurrency(totals.total_cost)}</span>
                                        </div>
                                    </div>
                                </div>
                            )}
                ...
                {!reportData && !loading && (
                    <div className="bg-primary rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-12 text-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400">{rt("Use the filters above to generate a production report")}</p>
                    </div>
                )}
