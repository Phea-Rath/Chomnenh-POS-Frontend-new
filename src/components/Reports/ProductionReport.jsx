...
const ProductionReport = () => {
    ...
    async function fetchReport() {
        try {
            setLoading(true);
            const res = await getProductionReport({ itemData: formData, token });
                        ...
                        <div className="overflow-x-auto print:overflow-visible print:p-10" ref={reportRef}>
                            ...
                            <table className="min-w-full border-collapse border border-gray-400">
                                <thead className="bg-gray-50">
                                    <tr>
                                        <th className="border border-gray-300 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Production No</th>
                                        ...
                                        <th className="border border-gray-300 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Qty</th>
                                        ...
                                        <th className="border border-gray-300 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Raw Materials</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {reportData?.map((item, index) => (
                                        <tr key={index} className="hover:bg-gray-50 !text-xs align-top">
                                            <td className="border border-gray-300 px-6 py-4 whitespace-nowrap font-medium text-gray-900">{item.production_no}</td>
                                            <td className="border border-gray-300 px-6 py-4 whitespace-nowrap text-gray-500">{item.production_date}</td>
                                            <td className="border border-gray-300 px-6 py-4 whitespace-nowrap text-gray-500">{item.barcode}</td>
                                            ...
                                            <td className="border border-gray-300 px-6 py-4 whitespace-nowrap text-gray-500">{item.created_by}</td>
                                            <td className="border border-gray-300 px-6 py-4 text-gray-500 min-w-[340px]">
                                                {item.details?.length > 0 ? (
                                                    <div className="space-y-2">
                                                        {item.details.map((detail, dIndex) => (
                                                            <div key={`${item.production_no}-${dIndex}`} className="border border-gray-200 rounded-md p-2">
                                                                <div className="font-medium text-gray-700">
                                                                    {detail.raw_material_name} ({detail.raw_material_code})
                                                                </div>
                                                                <div className="text-gray-500">
                                                                    Qty: {formatNumber(detail.quantity)} | Unit Cost: {formatCurrency(detail.cost_per_unit)} | Total: {formatCurrency(detail.total_cost)}
                                                                </div>
                                                            </div>
                                                        ))}
                                                    </div>
                            ...
                            {reportData?.length > 0 && (
                                <div className="mt-6 p-4 bg-gray-50 rounded-md">
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        <div>
                                            <span className="font-medium text-gray-700">Total Productions: </span>
