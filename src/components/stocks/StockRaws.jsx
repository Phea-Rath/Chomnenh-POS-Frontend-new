...
const StockRaws = () => {
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            if (res.data.status === 200) {
                refetch();
                saleRefetch();
                toast.success('Raw stock record deleted successfully!');
            }
        } catch (error) {
            toast.error(error.message || 'Failed to delete raw stock record!');
        } finally {
            setLoading(false);
        }
    };
            ...
            dataToExport.forEach((stock) => {
                getStockItems(stock).forEach((item) => {
                    itemsData.push({
                        ...
                        'Unit Cost': item.item_cost,
                        'Total Value': (item.quantity || 0) * (item.item_cost || 0),
                        'Expiry Date': item.expire_date || 'N/A',
    ...
    const EmptyState = () => (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">No Raw Stock Records Found</h3>
            <p className="text-gray-500 text-lg max-w-md mx-auto">
                {stocks.length === 0
                    ? 'Start tracking your raw material movements by creating your first stock record'
            ...
            {stocks.length === 0 && (
                <Link to="add">
                    <button className="mt-6 px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-lg hover:from-blue-700 hover:to-purple-700 flex items-center gap-2 mx-auto">
                        <FaPlus /> Create First Raw Stock Record
                    </button>
                </Link>
            )}
        </div>
    ...
    const LoadingState = () => (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center">
            <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-gray-600 text-lg font-medium">Loading raw stock data...</p>
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 mb-2">
                            Raw Stock <span className="text-blue-600">Inventory</span>
                        </h1>
                        <p className="text-gray-600">Track and manage raw material movements and operations</p>
                    </div>
