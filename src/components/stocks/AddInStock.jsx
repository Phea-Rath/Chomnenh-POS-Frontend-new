...
import { useGetAllStockQuery, useGetStockByIdQuery, useCreateStockMutation, useUpdateStockMutation } from "@/features/stocks/stocksSlice";
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (response?.status == 200 || response?.data?.status == 200 || response) {
        refetch();
        if (isEditMode && refetchStock) refetchStock();
        if (saleItemContext?.refetch) saleItemContext.refetch();
        if (refetchItems) refetchItems();
        if (refetchSales) refetchSales();
        if (wasteRefetch) wasteRefetch();
        setLoading(false);
        notify.success(
          response?.data?.message || response?.message || (isEditMode ? t('stockUpdatedSuccess') : t('stockCreatedSuccess'))
        );
        const targetId = response?.data?.id || response?.id || id;
        navigator(`/stock-invoice/${targetId}`);
      }
    } catch (error) {
      setLoading(false);
