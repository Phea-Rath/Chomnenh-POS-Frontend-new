...
import { useCreateStockTypeMutation, useGetAllStockTypesQuery } from "@/features/stocks/stockTypesSlice";
...
const CreateStockTypes = ({ onAdd }) => {
  ...
  async function handleConfirm() {
    try {
      setAlertBox(false);
      setLoading(true);
      await createStockType({ itemData: stock_types, token }).unwrap();
      toast.success(vt('Stock type created successfully'));
      onAdd();
    } catch (error) {
      toast.error(error?.data?.message || error?.message || vt('An error occurred while creating the stock type'));
    } finally {
      setLoading(false);
      setAlertBox(false);
    }
